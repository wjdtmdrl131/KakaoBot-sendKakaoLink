...
exports.Kakao = function () {
	...
	Kakao.prototype.init = function (apiKey) {
		if (typeof apiKey != "string" || apiKey.length != 32) {
			throw new Error(
				"API 키 " + apiKey + " 는 올바르지 않은 API 키 입니다."
			);
		}
		this.apiKey = apiKey;
		return true;
	...
	Kakao.prototype.static = {
		kakaoAgent:
			"sdk/1.36.6 os/javascript lang/en-US device/Win32 origin/http%3A%2F%2Fmagenta.kro.kr",
		userAgent:
	...
	Kakao.prototype.authApiKey = function () {
		...
		var response = org.jsoup.Jsoup.connect(
			...
			.header("User-Agent", this.static.userAgent)
			.data(authdata)
			.method(org.jsoup.Connection.Method.POST)
			.execute();
		if (response.statusCode() == 401) {
			throw new Error(
				"API 키가 올바르지 않습니다. API 키를 다시 확인해주세요."
			);
		}
		...
		Object.assign(this.cookies, { _kadu: response.cookie("_kadu"), _kadub: response.cookie("_kadub"), _maldive_oauth_webapp_session_key: response.cookie("_maldive_oauth_webapp_session_key") });
      //Log.d(obj.length.toString());
	...
	Kakao.prototype.login = function (email, password) {
		if (this.isInitalized() == true) {
			...
			throw new Error(
				"login 메소드가 SDK 를 초기화 하기 전에 호출되었습니다."
			);
		}
		...
		var logindata = {
			...
			continue: decodeURIComponent(
				this.loginReferer.split("continue=")[1]
			),
			third: "false",
			k: "true",
		};
		var response = org.jsoup.Jsoup.connect(
			...
			.header("User-Agent", this.static.userAgent)
			.header("Referer", this.loginReferer)
		...
		if (status == -450) {
			throw new Error(
				"email 이나 password 이가 올바르지 않습니다. email과 password를 다시 확인해주세요."
			);
		}
		if (status != 0) {
			throw new Error(
	...
	Kakao.prototype.templateCheck = function (template, type) {
		...
		if (response.statusCode() == 400) {
			throw new Error(
				"템플릿 객체가 올바르지 않습니다. 타 도메인이 있는 경우 카카오 개발자 설정에서 해당 Url을 추가해주세요."
			);
		}
		Object.assign(this.cookies, {
		...
		var parsedata = response.parse();
		this.parsedTemplate = JSON.parse(
			parsedata.select("#validatedTalkLink").attr("value")
		);
		this.csrf = parsedata
			.select("div")
			.last()
